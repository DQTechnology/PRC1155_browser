...
(
    ...
    `is_support_erc1155`            tinyint(1)  NOT NULL COMMENT '是否支持erc721接口： 0-不支持 1-支持',
    `is_support_erc1155_metadata`   tinyint(1)  NOT NULL COMMENT '是否支持metadata接口： 0-不支持 1-支持',
    `create_time`                   timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`                   timestamp   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
DROP TABLE IF EXISTS `token_721_inventory`;
CREATE TABLE `token_721_inventory`
(
    `id`                 bigint(20)   NOT NULL AUTO_INCREMENT COMMENT '自增id',
    `token_address`      varchar(64)  NOT NULL COMMENT '合约地址',
...
(
    ...
    `medium_image`       varchar(256)          DEFAULT NULL COMMENT '中等缩略图',
    PRIMARY KEY (`id`),
    UNIQUE KEY `token_address` (`token_address`, `token_id`, `owner`)
);
