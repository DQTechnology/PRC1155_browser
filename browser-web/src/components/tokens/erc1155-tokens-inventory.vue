    ...
    <div class="inventory-box" v-loading="loading">
      <div class="box-item" v-for="item in inventory" :key="item.tokenId">
        ...
        <p class="token-id ellipsis">
          #
          <router-link class="normal cursor" :to="get1155IdUrl(item.contract, item.tokenId)">{{ item.tokenId }}</router-link>
        </p>
...
export default {
  ...
  methods: {
    getInventory() {
      ...
      this.loading = true;
      apiService.tokens.token1155InventoryList(param).then((res) => {
