    ...
    <div class="information box-relative" v-loading="loading">
      ...
      >
        ...
        <Item :label="$t('tradeAbout.sender')">
          <router-link class="cursor normal ellipsis" :to="getAddressUrl(detailInfo.from)">
            {{ detailInfo.from }}
          </router-link>
        </Item>
        ...
        <Item :label="$t('tradeAbout.recipient')" v-if="detailInfo.txType != '4000'">
          ...
          <div class="isContract " v-if="detailInfo.receiveType == '1'">
            ...
            <!-- 否则显示合约地址 -->
            <router-link v-else class="cursor normal ellipsis" :to="getContractUrl(detailInfo.to)">
              {{ detailInfo.to }}
            </router-link>
          </div>
        </Item>
        ...
        <!-- 数额 -->
        <Item :label="$t('tradeAbout.amount')" v-if="detailInfo.txType != '4000'">
          {{ detailInfo.value | formatMoney }} LAT
        </Item>
        <!-- 锁仓 -->
        <template v-if="detailInfo.txType == '4000'">
          <Item :label="$t('tradeAbout.restrictedAccount')">
            <router-link class="cursor normal ellipsis" :to="getAddressUrl(detailInfo.rpaccount)">
              {{ detailInfo.rpaccount }}
            </router-link>
          </Item>
          ...
          <Item :label="$t('tradeAbout.restrictedPlan')">
            <ul class="restricted-plan">
              <li v-for="(item, index) in detailInfo.rpplan" :key="index">
                {{ item.epoch }} Epoch({{ item.blockNumber }})：{{ item.amount | formatMoney }}
                LAT
              </li>
            </ul>
          </Item>
      ...
      <List
        ...
        v-if="
          ...
          detailInfo.txType == '6' ||
          detailInfo.txType == '7' ||
          detailInfo.txType == '8' || // 721 创建
          detailInfo.txType == '9' || // 721 调用
      ...
      >
        ...
        <Item :label="$t('contract.contract')">
          ...
          <div
            ...
            v-if="detailInfo.txReceiptStatus == 1 || (detailInfo.txReceiptStatus == 0 && detailInfo.txType !== 1)"
          >
            <template>
              <el-tooltip class="item" effect="dark" placement="bottom">
                <div slot="content" class="delegate-msg">
            ...
            <router-link class="cursor normal ellipsis" :to="getContractUrl(detailInfo.to)">
              {{ detailInfo.to }}
            </router-link>
          </div>
          <!-- 合约创建失败 -->
        ...
        <!-- 价值 -->
        <Item :label="$t('tradeAbout.value')">{{ detailInfo.value | formatMoney }} LAT</Item>

        <!-- 合约类型 -->
        <Item :label="$t('contract.contractType')">{{ this.contractTypeTitle(detailInfo.contractType) }}</Item>
            ...
            <ul v-for="(item, index) in detailInfo.erc721Params" :key="index">
              ...
              <li>
                <img
                  v-pic-preview
                  class="token-pic"
                  :src="item.innerImage || require('@/assets/images/Alaya-cat-721.svg')"
                />
              </li>
            </ul>
          </div>
        ...
        <Item :label="$t('tradeAbout.rewardDetails')">
          ...
          <div>
            ...
            <p style="width: 100%" v-for="item in detailInfo.rewards" :key="item.verify" v-else>
              ...
              <!-- 从xxx节点 此处需要做样式-->
              <router-link class="cursor normal ellipsis rewardGap" :to="getNodeUrl(item.verify)">
                {{ item.nodeName }}
              </router-link>
              <span class="rewardGap">{{ $t('tradeAbout.claimRewards') }}</span>
              <span class="rewardGap Gilroy-Medium">{{ item.reward | formatMoney }} LAT</span>
            </p>
          </div>
        </Item>
        <Item :label="$t('tradeAbout.transactionFee')">
      ...
      >
        ...
        <Item
          :label="
            $t(
              'tradeAbout.' +
                (detailInfo.txType == '2000' || detailInfo.txType == '2001' || detailInfo.txType == '2002'
                  ? 'proposer'
                  : 'validator')
            )
          "
        ...
        <template v-if="detailInfo.txType != '2004'">
          ...
          <Item :label="$t('tradeAbout.proposalType')">
            ...
            <span v-else-if="detailInfo.proposalOption">{{ $t('proposalOption.' + detailInfo.proposalOption) }}</span>
          </Item>
          <!-- 提案ID -->
          ...
          <Item :label="$t('tradeAbout.proposalTitle')">
            ...
            >
              {{ $t('createType.' + [detailInfo.txType]) }}-{{ detailInfo.pipNum }}
            </router-link>
            <router-link
              class="cursor normal ellipsis"
              :to="getProposalUrl(detailInfo.proposalHash)"
            ...
            >
              {{ $t('proposalOption.' + [detailInfo.proposalOption]) }}-{{ detailInfo.pipNum }}
            </router-link>
          </Item>
        </template>
        ...
        <template v-if="detailInfo.txType == '3000'">
          ...
          <Item :label="$t('tradeAbout.reportResult')">
            <span class="green vote-status" :class="{ 'red-status': detailInfo.reportStatus == 1 }">
              {{ detailInfo.reportStatus == 2 ? $t('tradeAbout.success') : $t('tradeAbout.fail') }}
            </span>
          </Item>
          <!-- 举报奖励 -->
        ...
        <template v-if="detailInfo.txType == '1003'">
          ...
          <Item :label="$t('tradeAbout.returnAmount')">
            ...
            <span class="letUp" v-else>
              ({{ $t('tradeAbout.remain') }}:{{ detailInfo.redeemLocked | formatMoney }} LAT)
            </span>
          </Item>
          <!-- 预计到账区块（退出验证人特有） -->
          <Item
